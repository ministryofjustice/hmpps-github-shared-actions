...
sev_lookup = {'high': 'error', 'moderate': 'warning', 'low': 'note'}
...
def main():
  ...
  for each_result_key in results_dict.keys():
    ...
    if not isinstance(
      via, str
    ):  # some vulnerabilities come via others - no sarif for them
      rules_dict = {
        'id': via['cwe'][0],
        'name': 'LanguageSpecificPackageVulnerability',
        'shortDescription': {'text': via['title']},
      ...
      result_dict = {
        ...
        'locations': [
          {'physicalLocation': {'artifactLocation': {'uri': this_result['name']}}}
        ],
      }

...
process_sarif() {
  ...
  jq --slurpfile snyk snyk-results.json '
    def docs:
      if (($snyk | length) == 0) then
        {}
      else
        ($snyk[0] // {})
      end;
    def docs_roots:
      (docs | if type == "array" then .[] else . end | select(type == "object"));
    def id_list($v; $k):
      if ($v | type) == "object"
        and (($v.identifiers // null) | type) == "object"
        and (($v.identifiers[$k] // null) | type) == "array"
      then
        $v.identifiers[$k]
      else
        []
    ...
    def ui_tags($v):
      (
        ((id_list($v; "CVE")) | map("cve:" + tostring))
        + ((id_list($v; "CWE")) | map("cwe:" + tostring))
        + ((id_list($v; "GHSA")) | map("ghsa:" + tostring))
      ) | unique;
...
prepare_summary() {
  ...
  if [[ -s snyk-results.json ]]; then
    json_counts=$(jq -r '
      ...
      def norm_sev($v):
        def obj_get($o; $k):
          if ($o | type) == "object" then $o[$k] else empty end;
        if ($v | type) != "object" then
          "unknown"
        else
        (
          obj_get($v; "severity")
          // obj_get($v; "effectiveSeverity")
          // obj_get(obj_get($v; "issueData"); "severity")
          // obj_get($v; "severityLevel")
          // obj_get(obj_get($v; "metadata"); "severity")
          // "unknown"
        )
        | tostring
        | ascii_downcase
        | if . == "moderate" then "medium"
          elif . == "error" then "high"
          elif . == "warning" then "medium"
          elif . == "note" then "low"
          elif . == "none" then "low"
          else .
    ...
    top_findings=$(jq -r '
      ...
      [
        ...
        | {
            ...
            cve: ((id_list(.; "CVE")) | map(tostring) | unique | join(", ")),

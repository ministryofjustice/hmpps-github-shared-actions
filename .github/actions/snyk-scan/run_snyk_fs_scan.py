...
def parse_snyk_major_minor(snyk_version_output: str) -> Tuple[Optional[int], Optional[int]]:
...
def main() -> int:
    ...
    if gradle_manifest is not None:
        ...
        elif scan_location_gradlew.exists() and os.access(scan_location_gradlew, os.X_OK):
        ...
        if gradlew_cmd is not None:
            ...
            gradle_version_proc = run_command([str(gradlew_cmd), "-v"], capture=True, env=env)
            gradle_version_output = (gradle_version_proc.stdout or "") + (gradle_version_proc.stderr or "")
            gradle_major = parse_gradle_major(gradle_version_output)

            snyk_version_proc = run_command(["snyk", "--version"], capture=True, env=env)
            snyk_version_output = (snyk_version_proc.stdout or "") + (snyk_version_proc.stderr or "")
            ...
            if (
                ...
                print(f"Detected Gradle {gradle_major}.x with Snyk CLI {snyk_version_output.strip()}.")
    ...
    if (scan_location / "pyproject.toml").is_file() and (scan_location / "uv.lock").is_file():
    ...
    if use_all_projects:
        snyk_args = ["snyk", "test", str(scan_location), "--all-projects", *base_args[3:]]
    else:
        print("Gradle project detected; running Snyk without --all-projects to avoid Gradle 9 compatibility issue.")
        snyk_args = base_args
    ...
    if cmd_exit == 2:
        ...
        if use_all_projects:
            debug_args = ["snyk", "test", str(scan_location), "--all-projects", *debug_base_args[3:]]
        else:
